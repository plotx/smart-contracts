...
contract("Rewards-Market", async function(users) {
	describe("Scenario2", async () => {
		...
		it("Scenario 2:All losers, no winners", async () => {
			...
			await marketConfig.setNextOptionPrice(27);
			functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", 0, 7, ethAddress, 2*1e8, 3);
			await signAndExecuteMetaTx(
		      privateKeyList[userNumber],
		      users[userNumber],
		      functionSignature,
			...
			for(i=1;i<11;i++)
			{
				...
				assert.equal(reward[0]/1e8,0);
				assert.equal(reward[1]/1e8,0);
...
contract("Rewards-Market", async function(users) {
	describe("Scenario2", async () => {
		...
		it("Scenario 2:All losers, no winners", async () => {
			...
			{
				let reward = await allMarkets.getReturn(users[i],7);
				assert.equal((~~(reward[0]/1e5))/1000,1*userRewardPlot[i-1]);
				assert.equal((~~(reward[1]/1e5))/1000,1*userRewardEth[i-1]);
				if(reward[0]*1 > 0 || reward[1]*1 > 0) {
					let ethBalBefore = await web3.eth.getBalance(users[i]);
					let plotBalBefore = await plotusToken.balanceOf(users[i]);
					let plotEthUnused = await allMarkets.getUserUnusedBalance(users[i]);
					functionSignature = encode3("withdraw(uint,uint256,uint)", plotEthUnused[0].add(plotEthUnused[1]),plotEthUnused[2].add(plotEthUnused[3]), 100);
