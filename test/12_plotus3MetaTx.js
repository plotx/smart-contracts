...
contract("Rewards-Market", async function(users) {
	describe("Scenario3", async () => {
		...
		it("Scenario 3: All winners, no losers", async () => {
			...
			for(i=1; i<11;i++){
			    ...
			    let functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", totalDepositedPlot, 7, plotusToken.address, predictionVal[i]*1e8, winningOption);
				await marketConfig.setNextOptionPrice(winningOption*9);
			...
			for(i=1;i<11;i++)
			{
				let betPointUser = (await allMarkets.getUserPredictionPoints(users[i],7,winningOption))/1e5;
	...
	describe("Scenario5", async () => {
		...
		it("Scenario 5: All losers, no winners and Staked less than 1 ETH", async () => {
			let i;
			let totalDepositedPlot = toWei(1000);
			let predictionVal  = [0,20,50,100,70,30,100];
			let options=[0,2,3,2,3,3,2];
			for(i=1; i<7;i++){
				await plotusToken.transfer(users[i], toWei(2000));
			    await plotusToken.approve(allMarkets.address, toWei(100000), { from: users[i] });
			...
			for(i=1;i<7;i++)
			{
				...
				assert.equal(reward/1e8,0);

				let plotBalBefore = await plotusToken.balanceOf(users[i]);

				let pendingData = await allMarkets.getUserUnusedBalance(users[i]);
				if(pendingData[0]/1+pendingData[1]>0){
					functionSignature = encode3("withdraw(uint,uint)", pendingData[0].iadd(pendingData[1]), 100);
					await signAndExecuteMetaTx(
				      privateKeyList[i],
				      users[i],
				      functionSignature,
