...
contract("Rewards-Market", async function(users) {
	describe("Scenario3", async () => {
		...
		it("Scenario 3: All winners, no losers", async () => {
			...
			let functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", 0, 7, plotusToken.address, 100*1e8, 1);
			...
			await marketConfig.setNextOptionPrice(9);
			functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", 0, 7, plotusToken.address, 400*1e8, 1);
			await signAndExecuteMetaTx(
		      privateKeyList[userNumber],
		      users[userNumber],
		      functionSignature,
			...
			await marketConfig.setPrice(toWei(0.001));
			await marketConfig.setNextOptionPrice(9);
			functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", 0, 7, plotusToken.address, 210*1e8, 1);
			await signAndExecuteMetaTx(
		      privateKeyList[userNumber],
		      users[userNumber],
		      functionSignature,
			...
			await marketConfig.setNextOptionPrice(9);
			functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", 0, 7, plotusToken.address, 123*1e8, 1);
			await signAndExecuteMetaTx(
		      privateKeyList[userNumber],
		      users[userNumber],
		      functionSignature,
			...
			await marketConfig.setNextOptionPrice(9);
			functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", 0, 7, ethAddress, 3*1e8, 1);
			await signAndExecuteMetaTx(
		      privateKeyList[userNumber],
		      users[userNumber],
		      functionSignature,
	...
	describe("Scenario5", async () => {
		...
		it("Scenario 5: All losers, no winners and Staked less than 1 ETH", async () => {
			...
			let functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", 0, 8, ethAddress, 0.1*1e8, 2);
			await signAndExecuteMetaTx(
		      privateKeyList[userNumber],
		      users[userNumber],
		      functionSignature,
			...
			functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", 0, 8, ethAddress, 0.2*1e8, 3);
			await signAndExecuteMetaTx(
		      privateKeyList[userNumber],
		      users[userNumber],
		      functionSignature,
			...
			await marketConfig.setPrice(toWei(0.01));
			await marketConfig.setNextOptionPrice(18);
			functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", 0, 8, ethAddress, 0.1*1e8, 2);
			await signAndExecuteMetaTx(
		      privateKeyList[userNumber],
		      users[userNumber],
		      functionSignature,
			...
			await marketConfig.setPrice(toWei(0.045));
			await marketConfig.setNextOptionPrice(27);
			functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", 0, 8, ethAddress, 0.1*1e8, 3);
			await signAndExecuteMetaTx(
		      privateKeyList[userNumber],
		      users[userNumber],
		      functionSignature,
			...
			await marketConfig.setPrice(toWei(0.012));
			await marketConfig.setNextOptionPrice(18);
			functionSignature = encode3("depositAndPlacePrediction(uint,uint,address,uint64,uint256)", 0, 8, ethAddress, 0.2*1e8, 2);
			await signAndExecuteMetaTx(
		      privateKeyList[userNumber],
		      users[userNumber],
		      functionSignature,
			...
			for(i=1;i<7;i++)
			{
				...
				if(pendingData[0]/1+pendingData[1]>0 || pendingData[2]/1+pendingData[3]>0){
					functionSignature = encode3("withdraw(uint,uint256,uint)", pendingData[0].iadd(pendingData[1]),pendingData[2].iadd(pendingData[3]), 100);
					await signAndExecuteMetaTx(
				      privateKeyList[i],
				      users[i],
				      functionSignature,
				...
				let ethBalAfter = await web3.eth.getBalance(users[i]);
				let plotBalAfter = await plotusToken.balanceOf(users[i]);
				assert.equal(Math.round((ethBalAfter-ethBalBefore)/1e18),Math.round((unusedEth[i]-usedEth[i-1]/10)/1+reward[1]/1e8));
				assert.equal(Math.round((plotBalAfter-plotBalBefore)/1e18),Math.round((unusedPlot[i]-usedPlot[i-1])/1+reward[0]/1e8));
			}
